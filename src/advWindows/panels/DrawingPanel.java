...
public class DrawingPanel extends FileFilter implements ActionListener, MouseMotionListener, Runnable, WindowListener
{
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() instanceof Timer) {
			...
			this.g2.setColor(JColorChooser.showDialog(frame, "Color Picker", this.g2.getColor()));
		} else if(e.getActionCommand().equals("Change Size...")) {
			float ps = ((BasicStroke)this.g2.getStroke()).getLineWidth(), ns=ps;
			try
			{
				ns = Float.parseFloat(JOptionPane.showInputDialog("New size:", ns));
			}
			catch(NullPointerException npe)
			{
				ns=ps;
			}
			this.g2.setStroke(new BasicStroke(ns, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
	...
	private void setupMenuBar() {
		...
		JMenuItem cpicker = new JMenuItem("Pick Color...", 'C');
		cpicker.addActionListener(this);
		cpicker.setAccelerator(KeyStroke.getKeyStroke("ctrl P"));
		
		JMenuItem size = new JMenuItem("Change Size...", 'S');
		size.addActionListener(this);
		size.setAccelerator(KeyStroke.getKeyStroke("ctrl L"));
	...
	{
		this.frame.setFocusTraversalKeysEnabled(b);
		this.panel.setFocusTraversalKeysEnabled(b);
		this.imagePanel.setFocusTraversalKeysEnabled(b);
	}
}
