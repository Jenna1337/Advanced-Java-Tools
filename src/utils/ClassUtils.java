...
public class ClassUtils
{
	 ...
	 * @deprecated This method may call class methods  with values that are illegal for that method.
	 */
	@Deprecated
	public static boolean TestClass(Class<?> clazz)
	{
		...
		{
			System.out.println(method.toString().replaceAll("[A-Za-z\\.]+\\.","").replaceAll(" throws [\\w ,]+","").replace("public ","").replace("static ","").replace("final ","").replace("native ",""));
	 ...
	 * @deprecated This method may call the class method it is given with values that are illegal for that method.
	 */
	@Deprecated
	public static boolean testMethod(Method method, Class<?> clazz)
	{
		...
		if(method.toString().contains("wait") || method.toString().contains("notify"))
			return true;
		try{
