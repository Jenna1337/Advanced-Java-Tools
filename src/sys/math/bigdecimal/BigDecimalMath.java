...
/** BigDecimal special functions.
	@see <a href="https://java.net/projects/omskjug/sources/svn/content/trunk/OpenAHP/src/arithmetic/BigDecimalMath.java?rev=199">
	https://java.net/projects/omskjug/sources/svn/content/trunk/OpenAHP/src/arithmetic/BigDecimalMath.java?rev=199
	</a>
 */
public class BigDecimalMath {
	...
	static public BigDecimal root(final int n, final BigDecimal x) {
		 ...
		 */
		final BigDecimal eps = x.ulp().divide(x.multiply(new BigDecimal(""+(2 * n))), BigDecimal.ROUND_HALF_UP);
		for (;;) {
		...
		return s.round(new MathContext(x.precision()));
	} /* BigDecimalMath.root */
	...
	static public BigDecimal log(BigDecimal x) {
		...
		if (x.compareTo(BigDecimal.ZERO) < 0) {
			...
			BigDecimal zpown = z;
			BigDecimal eps = x.ulp().divide(x, BigDecimal.ROUND_HALF_UP).divide(new BigDecimal(2), BigDecimal.ROUND_HALF_UP);
			BigDecimal resul = z;
			for (int k = 2;; k++) {
			...
			return resul.round(new MathContext(x.precision()));
		} else {
			final double xDbl = x.doubleValue();
	...
	static public BigDecimal addRound(final BigDecimal x, final BigDecimal y) {
		...
		BigDecimal errR = y.ulp().divide(new BigDecimal(2)).abs().add(x.ulp().divide(new BigDecimal(2)).abs());
		MathContext mc = new MathContext(new Double(Math.log10((resul.divide(errR).divide(new BigDecimal(2)).abs().doubleValue()))).intValue());
